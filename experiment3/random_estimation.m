...
function random_estimation(subnum)

cd ~/Desktop/abacus/estimcards
...
settings.extra_jitter = 50;

settings.abacus.line_width = 10;
settings.abacus.bead_diam = 63;
...
while GetSecs - start_time < settings.experiment_time_limit;
  ...
  beads = convertQuantityToAbacus(settings.q(i),settings.abacus.lines);
  settings.q_num_beads(i) = size(beads,1);
  settings.resp_len = settings.num_digits(i);
  
  % show numbers and get response
  drawAbacusArrayJitteredCenter(ws,settings,settings.q(i));
  WaitSecs(settings.viewing_interval);
  showMask(ws,mask);
  [resp(i) rt1(i) rt2(i)] = readInResponse(ws,mask,settings,1);
  
  % check if we're right
  if resp(i) == settings.q_num_beads(i)
    correct(i) = 1; 
  ...
  save(['data/' num2str(subnum) '-ESTC.mat'],'settings','resp','rt1','rt2');
